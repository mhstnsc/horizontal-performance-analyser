 ...
 */
public final class TotalPackageTimeWriter
{
    ...
    @Parameter(names = {
            "-e",
            "--event"
    }, description = "Type of event used to generate the flamegraph", converter = EventType.EventTypeConverter.class)
    ...
    public TotalPackageTimeWriter(JfrParameters jfrParameters, OutputWriterParameters parameters)
    {
    ...
    public void process() throws Exception
    {
        ...
        for (IEvent event : view)
        {
            ...
            if (eventType.getName().equals(name))
            {
                ...
                if (flrStackTrace != null)
                {
                    ...
                    processStackTrace(flrStackTrace);
                }
                processedEvents++;
            }
        }
        ...
        final long totalEvents = processedEvents;

        try (Writer writer = outputFile != null ? new FileWriter(outputFile) : new PrintWriter(System.out);
             BufferedWriter bufferedWriter = new BufferedWriter(writer))
        {
            List<ImmutablePair<String,Integer>> totalHitsList =
            computeTopMethods(packageToTotalHits)
                    .stream()
                    .filter(stringIntegerImmutablePair -> stringIntegerImmutablePair.right * 100 / totalEvents > 5)
                            ...
                            packageCountPair ->
                            {
                                try
                                {
                                    bufferedWriter.write(
                                            String.format(
                                                    "%100s : sampls: %5d, total: %3d%%, own: %3d%% \n",
                                                    packageCountPair.left,
                                                    packageCountPair.right,
                                                    packageCountPair.right * 100 / totalEvents,
                                                    packageToOwnHits.getOrDefault(packageCountPair.left, new AtomicInteger()).get() * 100 / totalEvents
                                            )
                                    );
                                }
                                catch (IOException e)
            ...
            totalHitsList
                    .stream()
                    .map(stringIntegerImmutablePair -> stringIntegerImmutablePair.left)
                    .forEach(
                            s ->
                            {
                                try
                                {
                                    bufferedWriter.write(String.format(
                                            "\n-------------------------------=[ Children Details %s : Total: %d%%, Own: %d%%]=--------------------------\n",
                                            s,
                                            packageToTotalHits.getOrDefault(s, new AtomicInteger()).get() * 100 / totalEvents,
                                            packageToOwnHits.getOrDefault(s, new AtomicInteger()).get() * 100 / totalEvents
                                    ));
                                    bufferedWriter.write(computeTopMethods(packageToChildren.get(s))
                                            .stream()
                                            .limit(20)
                                            .map(methodCountPair -> String.format(
                                                    "\t%100s : %d%% ( %d )\n",
                                                    methodCountPair.left,
                                                    methodCountPair.right * 100 / totalEvents,
                                                    methodCountPair.right
                                            ))
    ...
    private List<ImmutablePair<String, Integer>> computeTopMethods(Map<String, AtomicInteger> topMethods)
    {
        if(topMethods == null)
        {
        ...
        return topMethods
                .keySet()
                .stream()
                .map(s -> ImmutablePair.of(s, topMethods.get(s).get()))
                .sorted(Comparator.<ImmutablePair<String,Integer>>comparingInt(o -> o.right).reversed())
                .collect(Collectors.toList());
    }


    private Map<String, AtomicInteger> packageToTotalHits = new HashMap<>();
    private Map<String, AtomicInteger> packageToOwnHits = new HashMap<>();
    private Map<String, Map<String, AtomicInteger>> packageToChildren = new HashMap<>();

    private Set<String> chainEnd = new HashSet<>(Arrays.asList(
            "org.jooq",
            "com.google.protobuf",
            "com.sun.crypto"
    ));

    private List<String> packageToSubpackages(IMCFrame frame)
    {
        List<String> subpackages = new ArrayList<>();

        String[] splitedPackageName = frame.getMethod().getPackageName().split("\\.");

        String accumulatedPackage = "";

        for (String subpackage : splitedPackageName)
        {
            if (accumulatedPackage.isEmpty()) // skip the first one because its anyway kind of obvious
            {
                accumulatedPackage += subpackage;
            }
            else
            {
                accumulatedPackage += "." + subpackage;
                subpackages.add(accumulatedPackage);
            }
        }

        subpackages.add(computeClassFullName(frame));
        subpackages.add(computeMethodFullName(frame));

        return subpackages;
    }

    private String computeMethodFullName(IMCFrame frame)
    {
        return frame.getMethod().getPackageName() + "." + frame.getMethod().getClassName() + "." + frame.getMethod().getMethodName();
    }

    private String computeClassFullName(IMCFrame frame)
    {
        return frame.getMethod().getPackageName() + "." + frame.getMethod().getClassName();
    }

    private void processStackTrace(FLRStackTrace flrStackTrace)
    {
        Set<String> frameHits = new HashSet<>();
        Set<String> ownHits = new HashSet<>();
        Map<String, AtomicInteger> currentAndChildHits = new HashMap<>();
        Map<String, Set<String>> childrenHits = new HashMap<>();

        for (IMCFrame frame : flrStackTrace.getFrames())
        {
            frameHits.addAll(packageToSubpackages(frame));
        }

        ownHits.addAll(packageToSubpackages(flrStackTrace.getValidTopFrame()));


//            perPackageMethod.computeIfAbsent(frame.getMethod().getPackageName(), s -> new HashMap<>())
//                            .computeIfAbsent(computeMethodFullName(frame), s -> new AtomicInteger())
//                            .incrementAndGet();

            // accumulate frame
        for (String tracehit : frameHits)
        {
            packageToTotalHits
                    .computeIfAbsent(tracehit, s1 -> new AtomicInteger())
                    .incrementAndGet();
        }
        for (String ownHit : ownHits)
        {
            packageToOwnHits
                    .computeIfAbsent(ownHit, s1 -> new AtomicInteger())
                    .incrementAndGet();
        }

        // compute set of parents to update with children

        Set<String> parentHits = new HashSet<>();
        if(flrStackTrace.getValidTopFrame() != null)
        {
            for (IMCFrame frame : flrStackTrace.getFrames())
            {
                if(frame != flrStackTrace.getValidTopFrame())
                {
                    parentHits.addAll(packageToSubpackages(frame));
                }
            }

            // for parents increment the top frame
            List<String> hitPackages = packageToSubpackages(flrStackTrace.getValidTopFrame());
            for (String parentHit : parentHits)
            {
                for (String hitPackage : hitPackages)
                {
                    packageToChildren.computeIfAbsent(parentHit, s -> new HashMap<>())
                                     .computeIfAbsent(hitPackage, s -> new AtomicInteger())
                                     .incrementAndGet();
                }
            }
        }
    ...
    private boolean filter(long eventStartTimestamp, long eventEndTimestamp)
    {
        ...
        {
            return eventEndTimestamp >= startTimestamp && eventEndTimestamp <= endTimestamp;
        }
    }
